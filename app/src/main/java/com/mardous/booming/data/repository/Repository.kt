...
class RealRepository(
    ...
    override suspend fun isSongFavorite(songId: Long): Boolean {
        ...
        // 2. ?? ∫À–ƒ–ﬁ∏¥£∫Œ™Õ¯¬ÁµÁÃ®Ã·π©∂µµ◊ºÏ≤È
        // ”…”⁄µÁÃ®¡˜ «–Èƒ‚∏Ë«˙£¨≤ª¥Ê‘⁄”⁄±æµÿ«˙ø‚÷˜±Ì÷–£¨‘≠…˙ª˘”⁄ SQL JOIN µƒ≤È—Øª·”¿‘∂∑µªÿ false°£
        // Œ“√«÷±Ω”øÁ±Ì∂¡»°°∞Œ“µƒ ’≤ÿ°±∏Ëµ•µƒÀ˘”– ˝æ›£¨Ω¯––ŒÔ¿Ì ID µƒ«ø∆•≈‰°£
        return try {
            // π˝¬À≥ˆÀ˘”–µƒÕ¯¬ÁµÁÃ®∑÷¿‡
            val allRadioPlaylists = playlistRepository.playlistsWithSongs(true)
                .filter { it.playlistEntity.playlistName.startsWith("[Radio]") }
                
            // À¯∂®√˚Œ™°∞Œ“µƒµÁÃ®°±ªÚ°∞Œ“µƒµÁÃ®¡–±Ì°±µƒ◊® Ù ’≤ÿº–
            val radioFavPlaylist = allRadioPlaylists.find { 
                val name = it.playlistEntity.playlistName
                name.contains("Œ“µƒµÁÃ®") || name.contains(" ’≤ÿ") 
            }
            
            // »Áπ˚‘⁄’‚∏ˆ◊® Ù¡–±Ì¿Ô’“µΩ¡Àµ±«∞µÁÃ®µƒ ID£¨¡¢º¥∏ÊÀﬂ UI ∫Õ≥µª˙µ„¡¡∫Ï–ƒ£°
    ...
    override suspend fun deleteMissingContent() {
        ...
        playlists.forEach { playlistWithSongs ->
            // üåü Ê†∏ÂøÉ‰øÆÂ§çÔºöÈÅáÂà∞ http/https ÂºÄÂ§¥ÁöÑÁîµÂè∞ÊµÅÁõ¥Êé•ÊîæË°åÔºåÁªù‰∏çÂΩì‰Ωú‰∏¢Â§±Êñá‰ª∂Âà†Èô§ÔºÅ
