...
import androidx.media3.session.MediaSessionService // ?? 融合作者更新：新增媒体按键接口包
...
class PlaybackService :
    ...
    private val lyricsRepository: LyricsRepository by inject() // ?? 必须保留的本地注入
    ...
    /** Ignore the transient unset duration a resumed player reports. */ // ?? 融合作者更新：新增官方注释
    ...
    // ?? 本地扩展核心：无状态秒级提取，100% 免疫 UI 延迟导致的查错歌
    ...
    // =========================================================================
    // ?? 融合作者更新：修复 Android 12+ / 车载等媒体按钮无法连接的鉴权 BUG
    // =========================================================================
    ...
    // =========================================================================
    // ?? 融合作者更新：重构 Widget 响应的数据注入分发 (Commit 59515cc)
    // =========================================================================
        ...
        if (events.contains(Player.EVENT_SHUFFLE_MODE_ENABLED_CHANGED)) {
            ...
            if (!events.contains(Player.EVENT_TIMELINE_CHANGED)) {
                ...
                if (player.shuffleModeEnabled && persistentStorage.restorationState.isRestored) {
                    ...
                    // Keep the staged start index while the queue is empty. // ?? 融合作者更新注释
    ...
    /** Only the fields that genuinely come from the player; the rest is [WidgetDataSource]'s. */ // ?? 融合作者更新：新增官方注释
    ...
    /** A command issued before the saved state lands has nothing to act on */ // ?? 融合作者更新：新增官方注释
    ...
    /** The write is debounced */ // ?? 融合作者更新：新增官方注释
