...
class PlaylistDetailFragment : AbsMainActivityFragment(R.layout.fragment_playlist_detail), ISongCallback {
    ...
    private fun checkIsEmpty() { binding.empty.isVisible = playlistSongAdapter?.isNullOrEmpty == true }

    private fun setupButtons() {
        binding.header.playAction.setOnClickListener { playlistSongAdapter?.dataSet?.let { playerViewModel.openQueue(it, shuffleMode = OpenShuffleMode.Off) } }
        binding.header.shuffleAction.setOnClickListener { playlistSongAdapter?.dataSet?.let { playerViewModel.openAndShuffleQueue(it) } }
        binding.header.searchAction?.setOnClickListener { findNavController().navigate(R.id.nav_search, searchArgs(playlist.playlistEntity.searchFilter(requireContext()))) }
    }

    private fun setupRecyclerView() {
        playlistSongAdapter = PlaylistSongAdapter(activity = mainActivity, dataSet = emptyList(), itemLayoutRes = R.layout.item_list, isLockDrag = Preferences.lockedPlaylists, callback = this)
        recyclerViewDragDropManager = RecyclerViewDragDropManager().also { dragDropManager ->
            dragDropManager.setOnItemDragEventListener(object : RecyclerViewDragDropManager.OnItemDragEventListener {
                ...
                override fun onItemDragFinished(fromPosition: Int, toPosition: Int, result: Boolean) { if (fromPosition != toPosition) playlistSongAdapter?.saveSongs(playlist.playlistEntity) }
    ...
    private val adapterDataObserver = object : RecyclerView.AdapterDataObserver() { override fun onChanged() = checkIsEmpty() }

    override fun onCreateMenu(menu: Menu, menuInflater: MenuInflater) { menuInflater.inflate(R.menu.menu_playlist_detail, menu) }

    override fun onPrepareMenu(menu: Menu) {
        if (playlist.playlistEntity.isFavorites(requireContext())) menu.removeItem(R.id.action_delete_playlist)
    ...
    override fun onMenuItemSelected(menuItem: MenuItem): Boolean {
        if (menuItem.itemId == R.id.action_export_playlist) {
            ...
            if (!currentSongs.isNullOrEmpty() && playlistName.isNotEmpty()) syncPlaylistToLocalM3u(playlistName, currentSongs, isManualExport = true)
            else Toast.makeText(requireContext(), "播放列表为空或名称无效，无法导出", Toast.LENGTH_SHORT).show()
            return true
        }
        ...
        return when (menuItem.itemId) {
            ...
            R.id.action_search -> { findNavController().navigate(R.id.nav_search, searchArgs(playlist.playlistEntity.searchFilter(requireContext()))); true }
            R.id.action_lock -> {
                val lockedPlaylists = !Preferences.lockedPlaylists
                Preferences.lockedPlaylists = lockedPlaylists
                menuItem.setIcon(if (lockedPlaylists) R.drawable.ic_lock_24dp else R.drawable.ic_lock_open_24dp)
                showToast(if (lockedPlaylists) R.string.playlist_locked else R.string.playlist_unlocked)
    ...
    override fun songMenuItemClick(song: Song, menuItem: MenuItem, sharedElements: Array<Pair<View, String>>?): Boolean {
        return when (menuItem.itemId) {
    ...
    override fun songsMenuItemClick(songs: List<Song>, menuItem: MenuItem) {
        when (menuItem.itemId) {
                    ...
                    viewLifecycleOwner.lifecycleScope.launch(Dispatchers.IO) {
                        ...
                        for (song in songs) {
                            ...
                            if (!result.lrcWithTrans.isNullOrBlank()) {
                                ...
                                } catch (e: Exception) { Log.e(TAG, "LRC 写入失败: ${song.title}", e) }
                            }
                        }
                        withContext(Dispatchers.Main) {
                            toast.cancel()
                ...
                if (songs.isNotEmpty()) {
                    ...
                    viewLifecycleOwner.lifecycleScope.launch(Dispatchers.IO) {
                        ...
                        withContext(Dispatchers.Main) {
                            ...
                            // 🌟 核心即时刷新：强制重置 dataSet 引用并全局重绘，彻底解决多选/批量获取后缩略图不显示的新图 Bug
                            playlistSongAdapter?.let { adapter ->
            ...
            R.id.action_pin_to_top -> { if (songs.isNotEmpty()) pinSongsToTop(songs) }
            R.id.action_unpin -> { if (songs.isNotEmpty()) unpinSongs(songs) }
